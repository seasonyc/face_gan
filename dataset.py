...
def load_celeba(data_dir, batch_size, prefetch_batch=1, num_threads=4, buffer_size=4096, part='train', consumer = 'vae', smallbatch = None):
    ...
    if part == 'test':
        ...
        img_paths = img_paths[:162770]  #162770
        if consumer == 'classifier' or consumer == 'translator':    
    ...
    def decode_and_preprocess_func(file, label=None):
        ...
        if consumer == 'classifier': 
            ...
            '''
            r = tf.random.uniform([], 0.0, 1.0)
            target_label = tf.cond(greater(r, tf.constant(0.5)),
                lambda:1.0,
                lambda:0.0)
            return (image, target_label), (image, target_label)
            '''
    ...
    if not smallbatch:
        ...
        return dataset.batch(smallbatch).take(1)
